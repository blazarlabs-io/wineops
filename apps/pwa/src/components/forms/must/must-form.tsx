...
export default function MustForm() {
  ...
  const [formData, setFormData] = useState<Must | undefined>(workingMust);
  ...
  useEffect(() => {
    if (selected.length > 0 && musts.length > 0) {
  ...
  useEffect(() => {
    ...
    const formatted = {
      ...
      status: workingMust?.status || MustStatus.NEW_MUST,
    } as Must;

    reset(formatted);
    setFormData(formatted);
  }, [reset, workingMust, musts?.length]);
