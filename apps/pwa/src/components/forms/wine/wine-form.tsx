...
export default function WineForm() {
  ...
  const [formData, setFormData] = useState<Wine | undefined>();
  ...
  useEffect(() => {
    const name = `Wine_${wines?.filter(({ rowType }) => rowType !== "group")?.length + 1}`;
    ...
    const formatted = {
      ...
      status: existingWine?.status || WineStatus.NEW_WINE,
    } as Wine;

    reset(formatted);
    setFormData(formatted);
  }, [reset, existingWine, wines]);
