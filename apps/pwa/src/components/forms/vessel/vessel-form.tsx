...
export default function VesselForm() {
  ...
  const [formData, setFormData] = useState<Vessel | undefined>(workingVessel);
  ...
  useEffect(() => {
    if (selected.length > 0 && vessels.length > 0) {
  ...
  useEffect(() => {
    ...
    const formatted = {
      ...
      ...(workingVessel?.type === VesselType.BARREL && {
        barrelInfo: {
          ...workingVessel?.barrelInfo,
          usageStatus:
            workingVessel?.barrelInfo?.usageStatus ||
