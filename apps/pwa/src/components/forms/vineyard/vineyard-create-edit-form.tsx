...
import { vineyardFormSchema } from "@/models/schemas/vineyard-schema";
import {
  Coordinates,
  DbResponse,
  FormMode,
...
const VineyardForm = forwardRef<VineyardFormRef, VineyardFormProps>(
  ({ setIsSubmitting, onNameError }, ref) => {
    const { user } = useAuth();
    ...
    const [identificatorUnicParcela, setIdentificatorUnicParcela] =
      useState("");
    ...
    const validateForm = useCallback(async () => {
      ...
      // If there's a name error, don't proceed
      if (hasNameError) {
        console.error("Name validation failed:", nameError);
        return false;
      }

      // Trigger validation and check results
      const isFormValid = await trigger();

      console.log("Form validation check [NEW]:", {
        hasNameError,
        nameError,
        isFormValid,
        formErrors: errors,
        overallValid: isFormValid && !hasNameError,
      });
                      ...
                      <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <FormControl>
                            ...
                            >
                              {wineColorOptions.map((grapeColor, index) => (
                                <MenuItem
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          <div className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-2 justify-between">
                              <div className="flex flex-col gap-2">
                                <InputLabel className="text-sm text-muted-foreground">
                                  Enter distance between vines on the same row
                                  (m)
                                </InputLabel>
                              </div>
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-2">
                                  <Checkbox
                                    ...
                                    checked={
                                      formData?.info?.certifications?.ice
                                        ?.active
                                    }
                                    onChange={(e) =>
                                      handleCheckboxChange(
