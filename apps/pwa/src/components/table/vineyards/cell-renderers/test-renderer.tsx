...
export const TestRenderer = (params: CustomCellRendererProps) => {
  return (
    <span>
      {getParts(params).map(
        ({ value: partValue, match, activeMatch }, index) =>
          match ? (
            <mark
              key={index}
              className={`ag-find-match${activeMatch ? " ag-find-active-match" : ""}`}
            >
              {partValue}
            </mark>
          ) : (
