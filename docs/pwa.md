...
### ag-Grid Flow (Grouping, Search, Pinning)
```mermaid
flowchart LR
  Data[Context data\n(e.g., vineyards, grapes, must, wine)\nwith id + group[] + rowType] --> Normalize[Normalize rowData\nensure group[] and rowType=item]
  Normalize --> Grid[AgGridReact\n(treeData mode)]
  ToolsBar[ToolsBar\n(search, pivot, pin, group dialogs)] --> Grid
  Grid --> Selection[SelectionChanged\nupdates useSelectedEntitiesStore]
  ToolsBar --> GroupDialogs[Group/Ungroup Dialogs]
  GroupDialogs --> UpdateGroup[updateRowsGroup\ncreates/moves groups]
  UpdateGroup --> Firestore[db[entityName].updateGroup\nWINERY/{uid}/collection]
  ToolsBar --> Pinning[Pin/Unpin\nusePinnedEntitiesStore]
  Pinning --> Grid
  ToolsBar --> Search[Find module\nvalue + next/prev]
  Search --> Grid
```
